...
export default function FoodStock() {
  ...
  const header = ['Cattle ID', 'Cattle Name', 'Cattle Color', 'Status', 'Temperature', 'Edit', 'Delete']
  // const value = 
  ...
  const upadteData = (index) => {
    ...
    if(index <= keys.length){
      ...
      let userRef = database().ref('health/' + data[index].key);
      ...
      let userRef = database().ref('/health').push(data[index])
      .then(res => {
        loadData()
        setEditIndex(-1)
